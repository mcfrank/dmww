...
That is, for an utterance of $N$ words there is a variable $r^{i} \in \{1\dots N+1\}$ that determines which word realizes the referential intention. 
...
\end{cases}
\end{equation}
Note that there is a chance that $r^{i}=N+1$ and as a result \emph{no} word in the utterance is referring.
