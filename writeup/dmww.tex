...
\end{equation}
Where $ob$ can be any object in the world and can also be the special ``object'' $NR$, which accounts for all non-referential words. %note: should we talk about them as objects or categories?
...
%note: do we need to give P(W^{i}_{j}|...) or is the stat notation enough?
...
We assume that the words, $\vec{W}$, and objects, $\vec{O}$, are fully observed, and hence the problem for the learner is to infer the lexicon, $L$, and the intentions and referential words, $\vec{I}$ and $\vec{r}$.

%Notes to myself: set up the learning problem of jointly inferring L,I,r, but then point out that it is only the predictive discs that really matter. Then make sure to point out that the distribution on L screens off I,r from predictions. Also point out that L has conjugate prior, so that if you knew I,r you have closed form for predictive P(w|o)... Use chain rule to write the distribution over next I,r in terms of previous observations?



The learner can jointly infer $\vec{I}$ and $L$ using Bayes' rule:
\begin{equation}
P( \vec{I}, L | \vec{W}, \vec{O}) \propto P(\vec{W}| \vec{I}, L, \vec{O} )  P(\vec{I},L | \vec{O}).
\end{equation}
%
Because of the independence structure of the generative model shown in Figure \ref{fig:genmod}:
\begin{equation}
P( \vec{I}, L | \vec{W}, \vec{O}) \propto P(\vec{W}| \vec{I}, L )  P(\vec{I} | \vec{O}) P(L).
\end{equation}
